import pygame, sys, random, time, os, psycopg2
from config import config
params = config()
pygame.init()
pygame.display.set_caption("Snake")
def Save(params, highscore, name):
    conn = psycopg2.connect(**params)
    cur = conn.cursor()
    cur.execute("UPDATE Snake SET score = %s WHERE name = %s", (highscore, name))
    conn.commit()
    cur.close()
    conn.close()
    ...
    def move(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                global params, highscore, name
                Save(params, highscore, name)
                pygame.quit()
                sys.exit()
...
try:
    ...
    scoretext = font.render("Highscore - " + str(highscore), True, (0,255,0))
except:
    cur.execute("INSERT INTO Snake(name, score) VALUES(%s,%s)", (name, 0))
    conn.commit()
    scoretext = font.render("Welcome", True, (0,255,0))
cur.close()
conn.close()
screen.blit(scoretext,(250-scoretext.get_width()//2,200-scoretext.get_height()//2))
...
while True:
    ...
    if Over: 
        ...
        scoretext = font.render("Highscore - " + str(highscore), True, (0,255,0))
        screen.fill((0,0,0))
        screen.blit(lose,(250-lose.get_width()//2,150-lose.get_height()//2))
        screen.blit(scoretext,(250-scoretext.get_width()//2,250-scoretext.get_height()//2))
        pygame.display.flip()
        time.sleep(3)
        pygame.quit()
        sys.exit()
