...
class Food:
    ...
    def new(self, S, W, F1, F2):
        self.x = random.randint(0,19)
        self.y = random.randint(0,19)
        q = [self.x, self.y]
        while q in S.body or q in W.body or (self.x == F1.x and self.y == F1.y) or (self.x == F2.x and self.y == F2.y):
            self.x = random.randint(0,19)
            self.y = random.randint(0,19)
            q = [self.x, self.y]
class Food1:
    def __init__(self, S, W, F):
        ...
        self.time = 30
        q = [self.x, self.y]
        while q in S.body or q in W.body or (self.x == F.x and self.y == F.y):
            self.x = random.randint(0,19)
            self.y = random.randint(0,19)
            q = [self.x, self.y]
    def draw(self, screen):
        pygame.draw.rect(screen, (0,0,255), pygame.Rect(self.x*20,self.y*20,20,20))
    def new(self, S, W, F, F2):
        self.x = random.randint(0,19)
        self.y = random.randint(0,19)
        q = [self.x, self.y]
        while q in S.body or q in W.body or (self.x == F.x and self.y == F.y) or (self.x == F2.x and self.y == F2.y):
            self.x = random.randint(0,19)
            self.y = random.randint(0,19)
            q = [self.x, self.y]
class Food2:
    def __init__(self, S, W, F, F1):
        self.x = random.randint(0,19)
        self.y = random.randint(0,19)
        self.time = 20
        q = [self.x, self.y]
        while q in S.body or q in W.body or (self.x == F.x and self.y == F.y) or (self.x == F1.x and self.y == F1.y):
            self.x = random.randint(0,19)
            self.y = random.randint(0,19)
            q = [self.x, self.y]
    def draw(self, screen):
